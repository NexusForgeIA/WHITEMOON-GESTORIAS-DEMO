/* =========================================================================
   Gestoría López Gómez — interacciones (sistema de WHITEMOON-YOGA-PILATES)
...
(() => {
  ...
  const rot = $("#rotWord");
  if (rot && !reduced) {
    const words = ["tu fiscalidad", "tu nómina", "tu herencia", "tu empresa", "cada trámite"];
    let i = 0;
    setInterval(() => {
  ...
  if (reveals.length && "IntersectionObserver" in window && !reduced) {
    ...
    /* Si se entra por un enlace profundo (#servicios, #faq…), se restaura la
